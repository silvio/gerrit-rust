...
fn history(y: &clap::ArgMatches, config: &config::Config) -> GGRResult<()> {
    ...
    for ci in changeinfos {
        ...
        for (revision, revisioninfo) in cirevisions {
            ...
            let dryrun = config.dry_run().clone();
...
fn history_fetch_from_repo(fetchinfo: &entities::FetchInfo, topic: &str, dryrun: bool) -> GGRResult<String>
{
    ...
    cb.credentials(|url, username, allowed| {
        ...
        let homefolder = env::home_dir().ok_or(git2::Error::from_str(&format!("set HOME environment variable for searching of netrc")))?;
...
fn history_find_repo_for_fetchinfo_url<'a>(main_repo: &'a git2::Repository, url: &String) -> GGRResult<OwnedOrRef<'a, git2::Repository>>
{
    ...
    for submodule in submodules {
        ...
        if found {
            ...
            match submodule.open() {
                ...
                Err(x) => {
                    return Err(format!("{}", x.message()).into())
                },
            }
        }
    }
